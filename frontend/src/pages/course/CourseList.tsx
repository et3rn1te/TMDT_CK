...
import {Star, BookOpen, User, Search, Filter} from 'lucide-react';
...
import {CategoryResponse} from "../../types/categoryTypes.ts"; // Use specific type
...
const CourseList = () => {
    ...
    const [selectedCategory, setSelectedCategory] = useState<number>(0); // Use category ID, 0 for 'all'
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                ...
                setCategories([{id: 0, name: 'Tất cả danh mục', description: ''}, ...fetchedCategories]); // Add 'All' option
            } catch (err) {
                console.error('Error fetching data for CourseList:', err);
                setError(err instanceof Error ? err.message : 'Đã xảy ra lỗi khi tải danh sách khóa học hoặc danh mục');
            } finally {
                setLoading(false);
            }
        };
    ...
    const filteredCourses = courses.filter(course => {
        const matchesSearch = course.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
            (course.sellerName?.toLowerCase() || '').includes(searchQuery.toLowerCase()); // Handle potential null sellerName

        // Match by category ID
        const matchesCategory = selectedCategory === 0 || course.categoryId === selectedCategory;
    ...
    const sortedCourses = [...filteredCourses].sort((a, b) => {
        switch (sortBy) {
            ...
            case 'newest':
            default:
                // Assuming 'id' can be used for 'newest' (higher ID means newer)
                return b.id - a.id;
        }
    });
                        ...
                        <div className="flex flex-col md:flex-row gap-4">
                            ...
                            <div className="flex-shrink-0">
                                <select
                                    ...
                                    onChange={(e) => setSelectedCategory(Number(e.target.value))} // Parse to number
                                >
